class EasterBreak {
    ...
    initializeDefaults() {
        // Set default dates based on Easter Break rules
        const easter = DateUtils.getNextEaster();
        const startDate = DateUtils.getThursdayBeforeEaster(easter.getFullYear());
        const exchangeDate = easter; // Easter Sunday
        const endDate = DateUtils.getMondayAfterEaster(easter.getFullYear());
            ...
            <div class="holiday-config">
                ...
                <div class="config-row" style="display: flex; justify-content: space-between; align-items: center;">
                    <label class="checkbox-container">
                        <input type="checkbox" id="easterObserved" class="holiday-checkbox" ${this.config.observed ? 'checked' : ''}>
                        <span class="checkmark"></span>
                        Holiday is Observed
                    </label>
                    <button type="button" class="section1-button" id="easterNotesBtn" style="font-size: 12px; padding: 6px 12px;">
                ...
                <div class="config-row">
                    ...
                    <div class="datetime-container">
                        <input type="date" id="easterStartDate" class="config-date-input" value="${DateUtils.dateToISOString(this.config.startDate)}">
                        <input type="time" id="easterStartTime" class="config-time-input" value="${this.config.startTime}">
                    </div>
                </div>
                ...
                <div class="config-row">
                    ...
                    <div class="datetime-container">
                        <input type="date" id="easterExchangeDate" class="config-date-input" value="${DateUtils.dateToISOString(this.config.exchangeDate)}">
                        <input type="time" id="easterExchangeTime" class="config-time-input" value="${this.config.exchangeTime}">
                    </div>
                </div>
                ...
                <div class="config-row">
                    ...
                    <div class="datetime-container">
                        <input type="date" id="easterEndDate" class="config-date-input" value="${DateUtils.dateToISOString(this.config.endDate)}">
                        <input type="time" id="easterEndTime" class="config-time-input" value="${this.config.endTime}">
                    </div>
                </div>
                ...
                <div class="config-row">
                    ...
                    <div class="dropdown-container">
                        <select id="easterFirstHalfParent" class="config-select">
                        ...
                        <select id="easterFirstHalfYears" class="config-select">
                ...
                <div class="config-row">
                    ...
                    <div class="dropdown-container">
                        <select id="easterSecondHalfParent" class="config-select">
                        ...
                        <span class="dropdown-text">in</span>
                        <select id="easterSecondHalfYears" class="config-select">
    ...
    setupEventListeners() {
        // Notes button
        const notesBtn = document.getElementById('easterNotesBtn');
        if (notesBtn) {
        ...
        // Observed checkbox
        const observedEl = document.getElementById('easterObserved');
        if (observedEl) {
        ...
        // Start date - auto-calculate other dates based on Easter
        const startDateEl = document.getElementById('easterStartDate');
        if (startDateEl) {
            startDateEl.addEventListener('change', (e) => {
                ...
                // Determine what year we're dealing with and recalculate Easter dates
                const year = newStartDate.getFullYear();
                const easter = DateUtils.calculateEaster(year);
                const endDate = DateUtils.getMondayAfterEaster(year);
                
                this.config.exchangeDate = easter;
                this.config.endDate = endDate;
                
                // Update form fields
                document.getElementById('easterExchangeDate').value = DateUtils.dateToISOString(easter);
                document.getElementById('easterEndDate').value = DateUtils.dateToISOString(endDate);
        ...
        const exchangeDateEl = document.getElementById('easterExchangeDate');
        if (exchangeDateEl) {
        ...
        // End date
        const endDateEl = document.getElementById('easterEndDate');
        if (endDateEl) {
        ...
        // Time fields
        ['easterStartTime', 'easterExchangeTime', 'easterEndTime'].forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.addEventListener('change', (e) => {
                    const field = id.replace('easter', '');
        ...
        // Dropdown fields
        ['easterFirstHalfParent', 'easterFirstHalfYears', 'easterSecondHalfParent', 'easterSecondHalfYears'].forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.addEventListener('change', (e) => {
                    const field = id.replace('easter', '').charAt(0).toLowerCase() + id.replace('easter', '').slice(1);
    ...
    openNotesModal() {
        if (window.timesharingCalendar && window.timesharingCalendar.openNotesModal) {
            window.timesharingCalendar.currentNotesContext = 'holiday-easterBreak';
            window.timesharingCalendar.openNotesModal(
                'holiday-easterBreak',
                'Easter Weekend - Additional Notes',
    ...
    getInfoForDate(date) {
        // Check if Easter Break is observed
        ...
        // Check if date is within Easter Break period
        ...
        if (dateOnly.getTime() === startDateOnly.getTime()) {
            ...
            return {
                statusText: `To ${firstHalfParent} for Easter Break`,
            ...
            return {
                statusText: `With ${firstHalfParent} for Easter Break`,
                time: null,
                colorClass: colorClass,
                icon: '🐰'
            };
            ...
            return {
                statusText: `To ${secondHalfParent} for Easter Break`,
            ...
            return {
                statusText: `With ${secondHalfParent} for Easter Break`,
                time: null,
                colorClass: colorClass,
                icon: '🐰'
            };
            ...
            // Get the regular schedule for the day after Easter Break ends
            ...
            return {
                statusText: `To ${nextParent} (Easter Break Ends)`,
